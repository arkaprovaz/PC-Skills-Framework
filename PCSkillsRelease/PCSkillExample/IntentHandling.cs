...
namespace PCSkillExample
{
    public partial class IRCExample : Form
    {
        ...
        private static Dictionary<string, Func<IRCExample, string, string, string, string>> RequestHandlers =
            new Dictionary<string, Func<IRCExample, string, string, string, string>>()
            {
               ...
               // { "SearchPCIntent",  ((thisObj, target, topic, requestbody) => thisObj.PCSearch(target,topic,requestbody))},
                { "TweetHelloWorld",  ((thisObj, target, topic, requestbody) => thisObj.TwitterDemo(target,topic,requestbody))},
                { "InstDependencies",  ((thisObj, target, topic, requestbody) => thisObj.InstDependencies(target,topic,requestbody))},
                { "TweetCamera",  ((thisObj, target, topic, requestbody) => thisObj.TweetCamera(target,topic,requestbody))},
                { "PCInfo",  ((thisObj, target, topic, requestbody) => thisObj.PCInfo(target,topic,requestbody))},
                { "TweetIntent",  ((thisObj, target, topic, requestbody) => thisObj.TweetText(target,topic,requestbody))},
                { "TweetWithMedia",  ((thisObj, target, topic, requestbody) => thisObj.TweetMedia(target,topic,requestbody))}
        ...
        //private string InitiateAction(string target, string topic, string requestbody)
        //{
        //    object response = irc.ProgressDialog(requestbody);
        //    if (response == null)
        //    {
        ...
        //        value = (string.IsNullOrEmpty(value)) ? "" : value.Replace(" ", string.Empty);
        //        OnLogMessage(string.Format("PCGraphics-> Task Name: {0}", value), null, true);
        //        //Configure Wifi settings
        //        if (string.Equals(value, "windows", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the Intel Windows Graphics Universal app.", "", true));
        //            InitiateAction("windows10GraphicsPanel");
        //        }
        ...
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Saving the dxdiag settings as xml file.", "", true));
        //            InitiateAction("directx");
        //        }
        ...
        //            InitiateAction("directxwhqloff");
        //        }

        //        //Configure privacy settings

        //        else if (string.Equals(value, "on", StringComparison.OrdinalIgnoreCase) || string.Equals(value, "apppermissions", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the Privacy settings.", "", true));
        //            InitiateAction("directxwhqlon");
        //        }


        //        //Configure display and multi moniotor

        //        else if (string.Equals(value, "diag", StringComparison.OrdinalIgnoreCase) || string.Equals(value, "multimonitor", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the " + value + " settings.", "", true));
        //            InitiateAction("directxg");
        //        }
        ...
        //        else if (string.Equals(value, "frame", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the Location settings.", "", true));
        //            InitiateAction("framec");
        //        }

        //        //ms-settings:dateandtime

        //        else if (string.Equals(value, "mode", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the Date and time settings.", "", true));
        //            InitiateAction("modem");
        //        }

        //        //Configure fonts for Accessibility

        //        else if (string.Equals(value, "lists", StringComparison.OrdinalIgnoreCase))
        //        {
        //            response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
        //                            irc.BuildSpeechletResponse(topic, "Sure. Opening the Accessibility settings.", "", true));
        //            InitiateAction("listm");
        //        }
        ...
        private string TweetText(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                string value = irc.GetSlotString(requestbody, "text");
                //value = (string.IsNullOrEmpty(value)) ? "" : value.Replace(" ", string.Empty);
                //OnLogMessage(string.Format("PCGraphics-> Task Name: {0}", value), null, true);
                //Configure Wifi settings
                //InitiateAction("windows10GraphicsPanel");
                ...
                jsondecoded = Newtonsoft.Json.JsonConvert.DeserializeObject(o2.ToString());
                string taskvalue = jsondecoded["tweetomatic"];
                taskvalue = taskvalue + " -t \"" + value + "\"";
                try
                {
                    Console.WriteLine(taskvalue);
                    process.StartInfo.Arguments = "/c " + taskvalue;
                    process.Start();
                    response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                                    irc.BuildSpeechletResponse(topic, "Tweet posted." + value, "", true));
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.ToString());
                    response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                                    irc.BuildSpeechletResponse(topic, "Sorry. Something went wrong", "", true));
                }
            }
            return (new JavaScriptSerializer()).Serialize(response);
        }

        private string TweetMedia(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                string value = irc.GetSlotString(requestbody, "text");
                value = (string.IsNullOrEmpty(value)) ? "" : value.Replace(" #PCSkills", string.Empty);
                Process process = new Process();
                //ProcessStartInfo startInfo = new ProcessStartInfo();
                //startInfo.WindowStyle = ProcessWindowStyle.Hidden;
                ...
                jsondecoded = Newtonsoft.Json.JsonConvert.DeserializeObject(o2.ToString());
                string taskvalue = jsondecoded["tweetomatic"];
                string taskvalue1 = jsondecoded["image_path"];
                taskvalue = taskvalue + " -i " + taskvalue1 + " -t \"" + value + "\"";
                try
                {
                    Console.WriteLine(taskvalue);
                    process.StartInfo.Arguments = "/c " + taskvalue;
        ...
        //Launch the Demo tweet program
        private string TwitterDemo(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                    irc.BuildSpeechletResponse(topic, "Sure. Posting the demo tweet.", "", true));
                    InitiateAction("demotweet");

            }
            return (new JavaScriptSerializer()).Serialize(response);
        }

        //Installing Dependencies with Alexa
        private string InstDependencies(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                    irc.BuildSpeechletResponse(topic, "Sure. Installing the dependencies on your pc.", "", true));
                InitiateAction("dependencies");

            }
            return (new JavaScriptSerializer()).Serialize(response);
        }

        //Launching the Tweet cam
        private string TweetCamera(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                    irc.BuildSpeechletResponse(topic, "Sure. Opening camera on your PC.", "", true));
                InitiateAction("tweetcam");
        ...
        //Launching the Tweet cam
        private string PCInfo(string target, string topic, string requestbody)
        {
            object response = irc.ProgressDialog(requestbody);
            if (response == null)
            {
                response = irc.BuildResponse(irc.GetSessionAttributes(requestbody),
                    irc.BuildSpeechletResponse(topic, "Sure. Posting the specifications of your PC.", "", true));
                InitiateAction("pcinfo");

            }
            return (new JavaScriptSerializer()).Serialize(response);
        }

        public static void InitiateAction(string taskname)
        {
            Process process = new Process();
            //ProcessStartInfo startInfo = new ProcessStartInfo();
            //startInfo.WindowStyle = ProcessWindowStyle.Hidden;
            ...
            jsondecoded = Newtonsoft.Json.JsonConvert.DeserializeObject(o2.ToString());
            string taskvalue = jsondecoded[taskname];
            try
            {
                process.StartInfo.Arguments = "/c " + taskvalue;
                process.Start();
